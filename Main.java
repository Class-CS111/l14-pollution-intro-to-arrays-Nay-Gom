...
class Main 
{
  ...
  public static void main(String[] args) 
  {
    ...
    double averageLevels = co2Levels[19] - co2Levels[0];    
    System.out.printf("From 2001 to 2020, the average atmospheric CO₂ levels across the globe has grown %.2f ppm%n", averageLevels);
  }

  public static void printGraph (double[] dataValue, int [] yearValue) {
    for(int i = 0; i < dataValue.length; i++) {
      System.out.print(yearValue[i] + "");
      Main.printBar(dataValue[i]);
    }
...
// @param data the CO2 level for a year to draw a bar graph, MUST be >= 360
